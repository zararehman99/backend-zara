...
import { Baby, Feed, PumpSession, HealthLog, SleepLog, TushLog } from "../models/baby.model";
...
export const deleteTushLog = async (req: Request, res: Response) => {
  try {
    const { babyId, tushLogId } = req.params
    const deleted = await TushLog.destroy({ where: { babyId, tushLogId } })
    if (deleted) {
      res.status(200).json({ message: 'Tush log deleted successfully' })
    } else {
      res.status(404).json({ message: 'Tush log not found' })
    }
  } catch (error) {
    console.error(error)
    res.status(500).json({ message: 'Could not delete tush log' })
  }
}